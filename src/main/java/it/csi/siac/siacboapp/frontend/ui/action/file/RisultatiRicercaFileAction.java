...
*/
package it.csi.siac.siacboapp.frontend.ui.action.file;
...
import it.csi.siac.siacboapp.frontend.ui.model.file.RisultatiRicercaFileModel;
...
//	@Autowired
//	private transient AzioniTipoFileService azioniTipoFileService;  
	...
	// FIXME
	public String azioneTipoFile() throws Exception {
		
		File file = model.getElementoByIndex();
		
		TipoFileHandler tipoFileHandler = super.getTipoFileHandler(file.getTipo().getCodice());

		Object retVal = tipoFileHandler.execute(model.getIndexAzioneTipoFile(), request, sessionHandler.getEnte().getUid(), file.getUid(), sessionHandler.getAnnoEsercizio(), file.getNome());
	...
	public boolean mostraAzione(File file, int indexAzioneTipoFile) {
		return CodiceStatoFile.ELABORATO.equals(file.getStatoFile().getCodiceEnum());
	}
}
