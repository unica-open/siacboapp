...
$.fn.simplePagination = function(options)
{
	...
	*/
	...
	return this.each(function()
	{
		...
		$(container_id + ' ' + settings.pagination_container).children().addClass(settings.filtered_class);
			
		// Show the appropriate items given the specific page_number
		function refresh_page(page_number, item_range_min, item_range_max)
		{
			hide_elements(excluded_items);
			hide_elements(items);
			show_elements(items.slice(item_range_min, item_range_max));
		}

		function show_elements(elems)
		{
			elems.removeClass(settings.hidden_class).addClass(settings.visible_class).show();
		}
	
		function hide_elements(elems)
		{
			elems.removeClass(settings.visible_class).addClass(settings.hidden_class).hide();
		}
		...
		{
			items = $(container_id + ' ' + settings.pagination_container).children('.' + settings.filtered_class);
			excluded_items = $(container_id + ' ' + settings.pagination_container).children().not('.' + settings.filtered_class);
		...
		function refresh_simple_pagination(page_number)
		{
			...
			var item_range_min = (page_number - 1) * items_per_page,
			...
			var navigation = $(container_id + ' .' + settings.html_prefix + '-navigation');
		...
		$(container_id).on("filtered", function(event)
		{
			if (event.query !== '')
				current_page = 1;

			refresh_simple_pagination(current_page); 
		});
	});
};
