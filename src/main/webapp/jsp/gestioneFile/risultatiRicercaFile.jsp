...
<s:if test="hasActionMessages()">
	...
	<tr>
		<td class="file-tipo"><s:property value="tipo.descrizione" /></td>
		<td class="file-codice"><s:property value="codice" /></td>
		<td><span class="file-index hide"><s:property value="#st.index" /></span><a class="file-nome" data-content="<s:property value="note" />" rel="popover" data-trigger="hover" href="#" data-original-title="Note"><s:property value="nome" /></a></td>
		<td class="file-stato"><s:property value="statoFile.descrizione" /></td>
		...
		<td class="tab_Right">
			<div class="btn-group">
				...
				<s:if test="tipo.azioneServizio != null" >
					<s:if test="daElaborare" >				
					<li><a  class="azione-file" href="#msgElabora" data-toggle="modal">elabora</a></li>
					</s:if>
				</s:if>
					
					<li><a href="<s:url method="download" action="risultatiRicercaFile"><s:param name="index" value="#st.index"/></s:url>">download</a></li> 	
			...
			<div class="row pagination_conth">
				  ...
				  <div class="span6">                               
					<div id="paginazione2" class="dataTables_paginate paging_bootstrap pagination">
					  <ul>
						<li><s:if test="not listaPaginata.primaPagina"><a href="<s:url method="cerca" action="risultatiRicercaFile"><s:param name="pagina" value="0"/></s:url>">&laquo; inizio</a></s:if><s:else><a href="#">&laquo; inizio</a></s:else></li>
						<li><s:if test="listaPaginata.hasPaginaPrecedente"><a href="<s:url method="cerca" action="risultatiRicercaFile"><s:param name="pagina" value="%{pagina-1}"/></s:url>">&laquo; prec</a></s:if><s:else><a href="#">&laquo; prec</a></s:else></li>
						
						<s:iterator id="p" begin="numeroPaginaInizio" end="numeroPaginaFine">
							<li <s:if test="%{(#p-1) eq listaPaginata.paginaCorrente}">class="active"</s:if>><a href="<s:url method="cerca" action="risultatiRicercaFile"><s:param name="pagina" value="%{#p-1}"/></s:url>"><s:property/></a></li>
						</s:iterator>
						
						<li><s:if test="listaPaginata.hasPaginaSuccessiva"><a href="<s:url method="cerca" action="risultatiRicercaFile"><s:param name="pagina" value="%{pagina + 1}"/></s:url>">succ &raquo;</a></s:if><s:else><a href="#">succ &raquo;</a></s:else></li>
						<li><s:if test="not listaPaginata.ultimaPagina"><a href="<s:url method="cerca" action="risultatiRicercaFile"><s:param name="pagina" value="%{listaPaginata.totalePagine-1}"/></s:url>">fine &raquo;</a></s:if><s:else><a href="#">fine &raquo;</a></s:else></li>              
					  </ul>
					</div>         
				  </div>  
			</div> 
...
<!-- Modal msgAnnulla -->
	<div id="msgAnnulla" class="modal hide fade modal-azione" tabindex="-1" role="dialog" aria-labelledby="msgAnnullaLabel" aria-hidden="true">
	  ...
	  <div class="modal-footer">
		...
		<a class="anchor-azione" href="<s:url forceAddSchemeHostAndPort="true" method="annulla" action="risultatiRicercaFile"/>"><span class="hide"></span></a>
	  </div>
	</div>  
<!--/modale annulla --> 
...
<div id="msgElabora" class="modal hide fade modal-azione" tabindex="-1" role="dialog" aria-labelledby="msgElaboraLabel" aria-hidden="true">	
	 <div class="modal-body">
	  ...
	  <div class="modal-footer">
		<button class="btn" data-dismiss="modal" aria-hidden="true">no, indietro</button>
		<button class="btn btn-primary conferma-azione" data-dismiss="modal" aria-hidden="true">si, prosegui</button>
		<a class="anchor-azione" href="<s:url forceAddSchemeHostAndPort="true" method="elabora" action="risultatiRicercaFile"/>"><span class="hide"></span></a>
	  </div>
</div>  
<!-- end Modal -->	 
...
<r:include url="/ris/servizi/siac/include/myCheckbox.html" resourceProvider="rp"/>
	...
	<script type="text/javascript" src="${jspath}gestioneFile/risultatiRicercaFile.js" charset="utf-8"></script>
